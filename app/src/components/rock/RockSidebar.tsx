...
export default function RockSidebar() {
  ...
  return (
    <aside className="hidden md:flex fixed left-0 top-0 bottom-0 z-40 flex-col py-8 h-screen w-64 rounded-r-[3rem] overflow-hidden bg-red-50 shadow-xl shadow-red-900/5">
      <div className="px-8 mb-10">
        <h1 className="text-xl font-black text-red-700 tracking-tight">Rock Legends AI</h1>
        <p className="text-xs text-on-surface-variant font-bold opacity-70">Every Legend Gets a Soundtrack</p>
      </div>
        ...
        {navItems.map((item) => (
          <Link
            ...
            className={`flex items-center gap-3 px-4 py-3 mx-4 transition-all duration-200 ${
              pathname.startsWith(item.href) && item.href !== "/dashboard" && item.href !== "/film"
      ...
      <div className="px-4 mt-auto space-y-2">
        <div className="bg-red-100/50 p-4 rounded-xl flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-red-200 flex items-center justify-center overflow-hidden">
            {user?.user_metadata?.avatar_url ? (
              <img src={user.user_metadata.avatar_url} alt="" className="w-full h-full object-cover" />
            ) : (
              <span className="material-symbols-outlined text-red-700">person</span>
            )}
          </div>
